...
public class WordPuzzle {
    ...
    public boolean toBeLabeled(int r, int c, boolean [][] blackBoxes){
        if(blackBoxes[r][c]==false&&((r==0||blackBoxes[r-1][c]==true)||(c==0||blackBoxes[r][c-1]==true))){
            return true;
        }
        else{
            return false;
    ...
    public WordPuzzle(boolean [][] blackBoxes){
        ...
        for(int i = 0; i<blackBoxes.length;i++){
            for(int k = 0; k<blackBoxes[0].length;k++){
                if(toBeLabeled(i, k, blackBoxes)){
                    puzzle[i][k]=new Box(blackBoxes[i][k],num);
                    num++;
                }
                else{
                    puzzle[i][k]=new Box(blackBoxes[i][k],0);
                }
            }
        }
    }
